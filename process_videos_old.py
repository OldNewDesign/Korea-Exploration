...
from video_intel import config, store, export_excel, export_guide
...
def process_one(item, args):
    ...
    meta = download.fetch(url, download_audio=not args.metadata_only)

    # 2) transcribe (best effort)
    transcript_text, transcript_lang = "", ""
    if not args.metadata_only and meta.get("audio_file"):
        try:
